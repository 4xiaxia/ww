...
const AgentAvatar: React.FC<AgentAvatarProps> = ({ state, volume }) => {
  ...
  // CN版本固定使用红色主题
  const ringColor = state === AgentState.LISTENING ? 'border-red-400' : state === AgentState.THINKING ? 'border-yellow-500' : 'border-red-200';
  const badgeIcon = '🇨🇳';
  const tagColor = 'text-red-600';
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      ...
      <div className="absolute top-0 right-6 z-40 animate-bounce-slow">
        <div className="bg-white p-1.5 rounded-full shadow-md border border-red-100 flex items-center justify-center">
            <i className="fas fa-heart text-sm animate-pulse text-red-500"></i>
        </div>
      </div>
      ...
      {/* Name Tag */}
      <div className="absolute bottom-2 z-20 bg-white/95 backdrop-blur-sm px-4 py-2 rounded-2xl shadow-lg border transform translate-y-8 flex items-center gap-3 min-w-[140px] border-red-100">
        ...
        <div className="flex flex-col">
            ...
            <span className="text-[10px] font-bold text-gray-400 leading-tight">东里专线服务中</span>
        </div>
      </div>
      ...
      {state !== AgentState.IDLE && (
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-4 bg-white px-3 py-1.5 rounded-2xl shadow-lg animate-bounce z-30 border border-gray-100 whitespace-nowrap">
              <span className="text-xs font-bold text-red-500">
                  {state === AgentState.LISTENING ? '正在录音...' : '思考中...'}
              </span>
          </div>
      )}
